#################################################
# HelloID-Conn-Prov-Target-SaltoSpace-Permissions-Zones-List
# List zone as permissions
# PowerShell V2
#################################################
...
try {
    #region Get zone
    $actionMessage = "querying zones"

    $getSaltozoneSplatParams = @{
    ...
    Write-Information "SQL Query: $($getSaltozoneSplatParams.SqlQuery | Out-String)"
    ...
    $saltoZones | ForEach-Object {
        # Shorten DisplayName to max. 100 chars
        $displayName = "Zone - $($_.name)"

...
$queryAccountSoftDelete = "UPDATE [$sqlDatabaseHelloId].[dbo].[$sqlDatabaseHelloIdAccountTable] SET [Action] = @Action WHERE ExtUserId = @ExtUserId;"
...
Try {    
    ...
    Switch ($lookupResult.count) {
         0 {
            ...
            Write-Verbose -Verbose "Account record for '$($p.displayName)' with ExtUserId '$($account.ExtUserId)' does not exist in the HelloID database.
            #Mark success here!
        } 1 {
            #$queryAccount = $queryAccountUpdate
            Write-Verbose -Verbose "Account record exists in the HelloID database. Soft deleting account record for person '$($p.displayName)' and ExtUserId '$($account.ExtUserId)'"
        } default {
     ...
     if (-Not($dryRun -eq $True)) {
        ...
        Write-Verbose -Verbose "Soft deleted account record for person '$($p.displayName)' and ExtUserId '$($account.ExtUserId)' succesfully"
     }
} catch {
